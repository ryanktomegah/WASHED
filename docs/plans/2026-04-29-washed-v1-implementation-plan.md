...
- [ ] Audit: transactional outbox, immutable Postgres `audit_events` event store, filtered operator replay endpoint, strict launch-critical contracts, and registered aggregate/actor/payload-key contracts for every current core event implemented.
